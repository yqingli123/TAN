...
def hard_example_mining(dist_mat, labels, return_inds=False):
    ...
    # import pdb; pdb.set_trace()
    is_pos_masked = is_pos.float() * (1-torch.eye(is_pos.shape[0])).cuda()
    # dist_ap, relative_p_inds = torch.max(
    #     dist_mat[is_pos.long()].contiguous().view(N, -1), 1, keepdim=True)
    dist_ap, relative_p_inds = torch.max(dist_mat*is_pos_masked.float().contiguous(), 1, keepdim=True)
    # `dist_an` means distance(anchor, negative)
    # both `dist_an` and `relative_n_inds` with shape [N, 1]
    # dist_an, relative_n_inds = torch.min(
    #     dist_mat[is_neg.long()].contiguous().view(N, -1), 1, keepdim=True)
    masked_dist_mat_neg = dist_mat * is_neg.float() + torch.max(dist_mat) * (1-is_neg.float())
    dist_an, relative_n_inds = torch.min(masked_dist_mat_neg.contiguous(), 1, keepdim=True)
    # shape [N]
...
class TripletLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=True):
        ...
        is_eol_mask = (1 - (labels==0).float()).view(-1)
        loss_masked = loss * is_eol_mask
        loss_masked_mean = loss_masked.sum() / is_eol_mask.sum()
        # calculate valid triplet
        valid_num = (loss_masked > 0).float().sum()
        valid_sum = is_eol_mask.sum()
        valid_percent = valid_num / valid_sum
        # import pdb; pdb.set_trace()
        semi_hard_num = ((loss<self.margin) & (loss>0.)).float().sum()
        hard_num = valid_num - semi_hard_num
        semi_hard_percent = semi_hard_num / valid_num

        return loss_masked_mean, valid_num, valid_sum, semi_hard_num, dist_ap, dist_an
